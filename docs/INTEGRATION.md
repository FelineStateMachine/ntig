...
Materialized ordinary refs remain advertised even while a newer state is pending: stock Git needs the true old OID to send a valid compare-and-swap update. HEAD is selected exclusively from authority, not main/master heuristics. A missing branch is an unborn symbolic HEAD, including an empty accepted refs map. If that branch exists at a stale OID, HEAD is suppressed until the accepted target arrives. A null accepted HEAD explicitly disables the heuristic.
