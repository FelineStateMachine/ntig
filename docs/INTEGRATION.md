...
From a clean, committed ntig checkout, run `npm ci`, `npm run check`, `npm run pack:check`, then `npm pack --pack-destination <artifact-directory>`. The prepack hook rebuilds the library; the package smoke test compares two archives byte-for-byte under the same toolchain and installs one into a fresh Node/TypeScript consumer. Different Node/npm versions can produce different gzip bytes despite identical uncompressed tar contents; record those toolchain versions and distinguish content equality from archive equality. Keep the source commit, archive checksum and vendored archive together in the consuming project's provenance record. Install that archive and commit the consumer lockfile. Do not use a mutable sibling `file:../ntig` dependency.
...
From 0.2.1, `wal.withReadSession(async scopedWal => { ... })` optionally reuses bounded immutable reads across one fully awaited request, including writes and post-publication promotion. Construct the accepted-state wrapper from `scopedWal` **inside** the callback and pass the same scoped repository to promotion. Keep the callback inside the host's existing authority fence. All root reads, authorization callbacks, integrity/Git validations and writes still run; every underlying write reaches the host reservation/metering adapter. Do not retain the scoped repository or use it from a response stream/background task after the callback returns. It closes even on error. Default cache limits are 2 MiB of retained payload and 512 entries, not a total request-memory cap. See [read efficiency and metadata budgets](READ-EFFICIENCY.md) for composition, measurements and safety boundaries.
...
The default limits remain 4 MiB per pack, 16 MiB unique packed history, 128 legacy commits, 1,024 refs, 4,096 object entries and 16 MiB decode work/retained object budget. Format 2 removes the history cap but permits at most 128 unique packs. HTTP bodies default to 4 MiB + 256 KiB; produced responses to 17 MiB. These limits interact conservatively; they do not promise every combination fits a Workers CPU/memory budget. Production peak memory and CPU have not been benchmarked.

**Format 1 stops all new writes at 128 commits**, including deletion/expiry cleanup. Upgraded readers/writers can explicitly migrate with `wal.checkpoint()`; existing 0.1.x consumers must not see a migrated root. Format 2 retains historical receipts through `lookupRecord(id)` rather than exposing all records in `Snapshot.records`. This is opt-in: deploying the new library does not migrate storage. Hosts may hide expired refs using `lookupPrTip=false`, but hiding or deleting a ref does not reclaim pack bytes. Failed/concurrent writes and superseded checkpoint metadata leave orphans outside committed pack limits; rate limits, storage reservations and operational inspection remain necessary. Do not enable bucket expiry on committed data. Selective pack reads, compaction and coordinated garbage collection remain the next storage milestones. bindws pricing is deferred; current metering hooks remain available without dictating storage design.
...
## GRASP extensions in 0.4.0

[GRASP extension building blocks](GRASP-EXTENSIONS.md) documents the strengthened PR wrapper and bounded outbound `fetchGitPack` API. Unknown PR uploads now require explicit opt-in, public deletion is denied, and metadata reads filter PR authority. Hosts retain event verification, expiry, destination approval, synchronization scheduling and publication authority.
